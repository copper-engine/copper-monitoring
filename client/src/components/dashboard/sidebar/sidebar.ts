...
export class SidebarComponent extends Vue {
    ...
    get links(): Link[] {
     ...
     return [
        new Link('Statistics', '/dashboard/homepage?host=' + this.$store.state.connectionSettings.host + '&port=' + this.$store.state.connectionSettings.port, 'mdi-chart-bar'),
        ...
        new Link('Workflow Repository', '/dashboard/workflow-repo?host=' + this.$store.state.connectionSettings.host + '&port=' + this.$store.state.connectionSettings.port, 'mdi-file'),
