...
export class StatusComponent extends Vue {
    ...
    get links(): Link[] {
        ...
        return [
            ...
            new Link('Workflow Repository', '/dashboard/workflowRepo/' + this.status.id + '?host=' + this.$store.state.connectionSettings.host + '&port=' + this.$store.state.connectionSettings.port, 'mdi-file')
        ];
    }
}