...
then
    ...
    echo "---------------------------------------------------------------------------------------------"
    echo ""
    cd "${PATH_TO_SCRIPT}/client" && npm install && npm run build && npm run deploy && cd ../server && ./gradlew distZip && cp build/distributions/copper-monitoring.zip ../docker/ && cd ../docker
elif [ $1 == '--dockerize' ]
then
    echo ""
    ...
    echo "---------------------------------------------------------------------------------------------"
    echo ""
    cd ../docker && docker build -t copper-monitoring .
else 
    echo ""
    echo "Script usage:"
